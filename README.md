...
另外走本地服务时，请求由服务端加上鉴权头发给模型商，页面自身的请求里只有请求体中的 Key，不会出现在浏览器发往模型商的请求头里。
（注意：本地服务本身只对**本机来源**开放 CORS，且请求体上限 256 KB。）
...
> 这是怎么判断的：页面启动后先对同源 `/api/chat` 发一次 `OPTIONS` 探测，
> 只有本地 `server.js` 会回一个 `X-Local-Server` 标记头；探测不到（静态托管上没有这个接口）
> 或探测失败，就整局走直连，不会再对着不存在的接口发请求。
...
│  └─ boot.js          装配：状态、事件委托、网络调用、启动自检、部署判定
└─ test/
   ├─ core.test.js     核心逻辑测试（24 条，纯断言、不碰 DOM）
   └─ boot.test.js     启动冒烟、接线与部署判定回归（28 条，用假 DOM 真跑一遍启动）
```
...
```bash
node --test              # 全部 52 条（零依赖，用 Node 18 起内置的 node:test）
node test/core.test.js   # 只看核心逻辑（24 条）
node test/boot.test.js   # 只看启动冒烟与接线（28 条）
```
...
**启动冒烟与接线（`test/boot.test.js`，28 条）** 在没有浏览器的环境里用最小假 DOM 真跑一遍启动流程：
...
- **部署判定**（3 条）：本地 http 走 `/api/chat` 代理、静态托管（https）探测不到本地服务时直连接口地址、
  `file://` 双击打开时同样直连——这条守的是一个真实踩过的坑：判断写成
  `protocol === 'http:' || protocol === 'https:'` 时条件恒为真，部署到 GitHub Pages 后
  依然会 POST 同源的 `/api/chat`（静态托管只回 405），README 承诺的"自动改为直连"从未生效；
- 本地存储不可用时只提示、不把界面卡死；
...
- **"一个场景的评分量表键名可读性差"这条描述不准确（待重写）**：那句
  `总分 = 四个维度换算为百分制后汇总` 其实不是 JSON 键名，而是 12 个场景 `scoring` 文本末尾的**散文说明行**
  （见 `js/scenarios.js` 各场景 scoring 尾部）；真正的维度键名是规范短名
  （`宣教内容完整性` / `通俗化表达` / `依从性引导与误区纠正` / `沟通态度与共情`），运行与渲染都正常。
  若将来要把量表结构化（`{ dimensions: [{ key, 锚点, 权重 }], total }` 再由 `core.buildScoringSystem()` 拼提示词），
  可以从这行入手统一生成，改动需重跑全部 12 个场景的验证，所以独立进行。
- **`server.js` 没有自动化测试**：它现在零用例覆盖（CI 只对它做 `node --check` 语法检查）。
  已经手工验证并通过的行为：畸形 URL（`/%ZZ`、`/%E0%A4%A`）返回 400 而不是拖垮进程、
  路径穿越 403、`/api/chat` 缺少 Key 返回 400、请求体超 256 KB 返回 413、
  CORS 只对本机来源回 `Access-Control-Allow-Origin`。下一步应补 `test/server.test.js` 把这些断言固化下来。
- **无真实对话冒烟记录**：目前做过三层验证——语法与静态检查（`node --check`、`node --test` 52 条）、
  HTTP 层（上一条列出的 200/400/403/413 与畸形 URL 不崩）、
